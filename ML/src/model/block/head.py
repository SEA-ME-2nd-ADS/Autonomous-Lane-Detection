...
from .roigather import ROIGather

from ..loss import focal_loss, iou_loss, seg_loss
from utils.anchor_generator import CLRerNetAnchorGenerator
from utils.assigner import assign

from utils.lane_util import Lane
...
class CLRerHead(nn.Module):
    def __init__(
        ...
        self.anchor_generator = CLRerNetAnchorGenerator()
        ...
        self.attention = ROIGather(in_channels = self.prior_feat_channels,
                                   num_priors = self.anchor_generator.num_priors,
                                   sample_points = self.sample_points,
                                   fc_hidden_dim = self.fc_hidden_dim,
                                   refine_layers = self.refine_layers)
        
        
        self.loss_cls = focal_loss.KorniaFocalLoss()
        self.loss_bbox = nn.SmoothL1Loss()
        self.loss_seg = seg_loss.CLRNetSegLoss()
        self.loss_iou = iou_loss.LaneIoULoss()
