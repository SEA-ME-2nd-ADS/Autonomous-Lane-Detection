import torch
from model.loss import  focal_loss, iou_loss, seg_loss
...
def assign(
    ...
    # classification cost
    cls_score = focal_loss.focal_loss(predictions[:, :2], targets[:, 1].long(), 0.25)
    num_priors = predictions.shape[0]
    num_targets = targets.shape[0]
    ...
    iou = iou_loss.LaneIoULoss(predictions[..., 6:], targets[..., 6:], img_w, aligned=False)
